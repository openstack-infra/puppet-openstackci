...
class openstackci::logserver (
  ...
  if $ara_middleware {
    package { 'ara':
      ...
      provider => 'openstack_pip',
      require  => [Package['build-essential'], Package['python-dev'], Package['libffi-dev'], Package['libssl-dev']],
      notify   => Service['httpd']
    }

    # TODO: Remove this once https://review.openstack.org/#/c/511992/ is released
    file { '/usr/lib/cgi-bin/wsgi_sqlite.py':
      ensure => present,
      owner  => 'root',
      group  => 'root',
      mode   => '0644',
      source => 'puppet:///modules/openstackci/wsgi_sqlite.py',
      notify => Service['httpd']
    }
  }
}
