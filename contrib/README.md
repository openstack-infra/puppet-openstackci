...
* A public log server such as an Apache server or Swift service to host static log files
  ...
  system can be located behind a firewall or within a VPN.
...
Modify it as you need and save it to a secure location.
All parameters are documented inside the `../manifests/single_node_ci.pp` which is
the top level puppet class that is used.
...
First, setup your puppet site.pp file by starting with the sample provided.
...
Instead, you should update them from a different clone, merge the changes to master, and push
...
After you have sucessfully built an image, start nodepool.
(If you don't have an image yet, this is one of the first actions nodepool
...
hiera data (`/etc/puppet/environments/single_node_ci_hiera.yaml`), or update
...
in your `single_node_ci_hiera`, delete the respective repository, e.g.
