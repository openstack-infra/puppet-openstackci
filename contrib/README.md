...
* A public log server such as an Apache server or Swift service to host static log files
  generated by jenkins jobs. At the end of every job, Jenkins will upload its log files
  to this server/service. It is considered external because it usually
  ...
  `publishers` will scp log files from the jenkins slave to the log server.
  Instructions to set this up
  are not yet available, but generally follow the basics of these instuctions to set up
  puppet and use this puppet class for openstack users: `../manifests/logserver.pp`.
  The result will be a server deployed like [http://logs.openstack.org/](http://logs.openstack.org)
...
First, set up these 3 files by starting with the samples provided.

    cp /etc/puppet/modules/openstackci/contrib/single_node_ci_site.pp /etc/puppet/manifests/site.pp
    cp /etc/puppet/modules/openstackci/contrib/single_node_ci_hiera.yaml /etc/puppet/hiera.yaml
    cp /etc/puppet/modules/openstackci/contrib/single_node_ci_data.yaml /etc/puppet/environments

At this point, you should not need to modify either of the first two files.
Modify `/etc/puppet/environments/single_node_ci_data.yaml` as you need using the parameter
documentation described in `../manifests/single_node_ci.pp` (which is
...
Save the credential uuid in your hiera data:
    ...
    Copy the id to the 'jenkins_credentials_id' value in  /etc/puppet/environments/single_node_ci_data.yaml
...
Enable basic Jenkins security:
    ...
    Choose a password.
    check 'Sign up'
    Save the password to the 'jenkins_password' value in /etc/puppet/environments/single_node_ci_data.yaml

Get the new 'jenkins' user API token:

    http://<host fqdn/ip>:8080/
    Manage Jenkins --> People --> Select user 'jenkins' --> configure --> Show API Token
    Save this token to the 'jenkins_api_key' value in /etc/puppet/environments/single_node_ci_data.yaml

Make a backup of your hiera data (`/etc/puppet/environments/single_node_ci_data.yaml`)
to a secure location on a different host.

Reconfigure your system to use Jenkins security settings stored in
`/etc/puppet/environments/single_node_ci_data.yaml`
...
hiera data (`/etc/puppet/environments/single_node_ci_data.yaml`), or update
the puppet files, run the same puppet command to update the host.

    sudo puppet apply --verbose /etc/puppet/manifests/site.pp

If you need to change the git url in your `project-config` or any other git urls
in your `single_node_ci_data`, delete the respective repository, e.g.
